...
/*
	...
	*/
	...
	$sql = " SELECT tb1.*, DAYOFYEAR(DATE_FORMAT(NOW(), '%Y-12-31')) num_day_of_year
			...
			*/
	$sql = "SELECT tb2.member_id, tb2.receipt_id,
	...
	WHERE (YEAR(tb2.payment_date) = 2018 AND MONTH(tb2.payment_date) = 10)
	...
	while(( $row_loan = $rs_loan->fetch_assoc() )) {
		$row_no++;
			...
			*/
			$row_loan['account_id'] = empty($row_loan['account_id']) ? '&nbsp;' : $row_loan['account_id'] ;
			echo "
			<tr>
				...
				<td>{$interest}</td>
				<td style='display: none1;'>{$row_loan['total_amount']}</td>
				<td style='display: none1;'>{$row_loan['payment_date']}</td>
				<td style='display: none1;'>{$row_loan['num_datediff']}</td>
			</tr>
