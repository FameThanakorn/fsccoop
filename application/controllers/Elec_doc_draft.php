...
class Elec_doc_draft extends CI_Controller {
    ...
    public function index() {
        ...
        $this->paginater_all->select('t1.user_id, t1.status, t1.created_at, t1.updated_at, t1.name, t1.id, t1.last_access, t3.user_name, t2.user_id as reviewer, t2.read_at as reviewer_read_at
                                        , t5.user_id as approver, t6.user_id as approve_draft, t6.read_at as approve_draft_read_at');
		$this->paginater_all->main_table('coop_ele_document as t1');
        $this->paginater_all->page_now($_GET["page"]);
        $this->paginater_all->where("t1.status in (2,7)
                                        AND (t1.user_id = '".$_SESSION['USER_ID']."'
                                            OR ((t2.user_id = '".$_SESSION['USER_ID']."') AND t1.status = 2)
                                            OR ((t6.user_id = '".$_SESSION['USER_ID']."') AND t1.status = 7))");
        ...
        foreach($row['data'] as $key => $data) {
            ...
            if(empty($data["reviewer_read_at"]) && !empty($data["reviewer"])) {
                $data_update = array();
                $data_update['read_at'] = $process_timestamp;
                $this->db->where('type',"reviewer");
                $this->db->where('user_id',$data["reviewer"]);
                $this->db->where('document_id',$data["id"]);
                $this->db->update('coop_ele_document_user',$data_update);
            } else if (empty($data["approve_draft_read_at"]) && !empty($data["approve_draft"])) {
                $data_update = array();
                $data_update['read_at'] = $process_timestamp;
                $this->db->where('type',"approve_draft");
                $this->db->where('user_id',$data["approve_draft"]);
                $this->db->where('document_id',$data["id"]);
                $this->db->update('coop_ele_document_user',$data_update);
            }
        }
    ...
    public function add_draft_document() {
        ...
        if(!empty($_POST)) {
            ...
            if(empty($_POST["document_id"])) {
                ...
                if(!empty($_POST["cc_user_ids"])) {
                    foreach($_POST["cc_user_ids"] as $user_id) {
                        $data_insert = array();
                        $data_insert['document_id'] =  $document_id;
                        $data_insert['group_id'] = null;
                        $data_insert['user_id'] = $user_id;
            ...
            echo "<script> document.location.href='".PROJECTPATH."/elec_doc_draft"."' </script>";
        }
        ...
        if(!empty($_GET["id"])) {
            ...
            $cc_users = $this->db->select("t2.user_id, t2.user_name")
                                        ->from("coop_ele_document_user as t1")
                                        ->join("coop_user as t2", "t1.user_id = t2.user_id", "inner")
                                        ->where("t1.document_id = '".$_GET["id"]."' AND t1.user_id is not null AND type = 'cc'")->get()->result_array();
            $arr_data["cc_users"] = $cc_users;
        }
