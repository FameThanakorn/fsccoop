...
class Elec_doc_my extends CI_Controller {
    ...
    public function index() {
        ...
        $x++;
        $join_arr[$x]['table'] = '(SELECT * FROM coop_ele_document_user WHERE user_id = "'.$_SESSION['USER_ID'].'" AND type = "owner") as t3';
        $join_arr[$x]['condition'] = 't1.id = t3.document_id';
        $join_arr[$x]['type'] = 'left';

        $this->paginater_all->type(DB_TYPE);
		$this->paginater_all->select('t1.user_id, t1.status, t1.created_at, t1.updated_at, t1.name, t1.id, t2.id as doc_user_id, t2.user_id as receiver, t2.read_at');
		$this->paginater_all->main_table('coop_ele_document as t1');
        $this->paginater_all->page_now($_GET["page"]);
        $this->paginater_all->where("t1.status = 5 AND (t3.user_id = '".$_SESSION['USER_ID']."' OR t2.user_id = '".$_SESSION['USER_ID']."')");
        ...
        foreach($row['data'] as $key => $data) {
            ...
            if($data["user_id"] != $_SESSION['USER_ID'] && empty($data["read_at"]) && !empty($data["receiver"])) {
                $data_update = array();
            ...
            $receivers = $this->db->select("t2.id, t2.read_at, t3.user_name")
                                    ->from("coop_ele_document as t1")
                                    ->join("coop_ele_document_user as t2", "t1.id = t2.document_id", "inner")
                                    ->join("coop_user as t3", "t2.user_id = t3.user_id", "left")
                                    ->where("t1.id = '".$data["id"]."' AND t2.type in ('receiver','cc')")
                                    ->get()->result_array();
            $row['data'][$key]["receivers"] = $receivers;
            ...
            foreach($receivers as $receiver) {
                if(!empty($receiver["read_at"])) {
                    $row['data'][$key]["read_user"][] = $receiver;
                    $read_name .= !empty($read_name) ? "<br>".$receiver["user_name"] : $receiver["user_name"];
                }
                $receiver_name .= !empty($receiver_name) ? "<br>".$receiver["user_name"] : $receiver["user_name"];
            }
            $row['data'][$key]["receiver_name"] = $receiver_name;
            $row['data'][$key]["read_name"] = $read_name;
        }
    ...
    public function review_my_document() {
        $arr_data = array();
        $process_timestamp = date('Y-m-d H:i:s');
