...
class Account extends CI_Controller {
    ...
    public function account_save() {
        ...
        $journal_ref = $_POST["journal_type"].($account_datetime_arr[2]-2500).sprintf('%02d',$account_datetime_arr[1]);
        ...
        if(empty($last_journal_ref_account)) {
            $journal_ref .= sprintf('%02d',$account_datetime_arr[0])."001";
        } else {
            $last_journal_ref = $last_journal_ref_account->journal_ref;
            $journal_ref .= !empty($last_journal_ref) ? sprintf('%02d',$account_datetime_arr[0]).(sprintf('%03d', substr($last_journal_ref, -3) + 1)): sprintf('%02d',$account_datetime_arr[0])."001";
        }
    ...
    function get_account_balance_data($data) {
        ...
        $prev_year_budgets[30005] = $profit_total;
        ...
        $year_budgets[30005] = $prev_year_budgets[30005] + $profit_total;
