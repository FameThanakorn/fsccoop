			...
			foreach($member_indexs AS $member_index){
				...
				if($infoData['deduct_code']=='LOAN'){
					...
					if($infoData['pay_type']=='principal'){
                        ...
                        $group_total_data[$member_group['lv'].'_'.$loan_type_code.'_principal'] += $infoData['sum_pay_amount'];
					} else {
                        ...
                        $group_total_data[$member_group['lv'].'_'.$loan_type_code.'_interest'] += $infoData['sum_pay_amount'];
					}
					$datas[$member_group['member_id']]['total'] += $infoData['sum_pay_amount'];
					$total_data['total_amount'] += $infoData['sum_pay_amount'];
                    $group_total_data[$member_group['lv'].'_total_amount'] += $infoData['sum_pay_amount'];
                    ...
                    $group_total_data[$member_group['lv'].'_'.$infoData['deduct_code']] += $infoData['sum_pay_amount'];
					$total_data['total_amount'] += $infoData['sum_pay_amount'];
                    $group_total_data[$member_group['lv'].'_total_amount'] += $infoData['sum_pay_amount'];
...
//        echo "</pre>";
	...
	function coop_report_send_deduction_preview(){
		...
		$member_where = " AND t4.member_status <> 3";
		if (!empty($_GET["mem_type"])){
			//if (is_array($_GET["mem_type"]) && !in_array("all", $_GET["mem_type"])){
			...
			$temp = array();
			$temp_mem_type = (is_array($_GET['mem_type'])) ? $_GET['mem_type'] : explode(',', $_GET['mem_type']);
			foreach ($temp_mem_type as $key => $value) {
				//array_push($temp, '"'.$value.'"');
				array_push($temp, $value);	
			}
			if(in_array("all",$temp)){
				$member_where .= "";
			}else{
		...
		$department_order = "t1.mem_group_id,t1.id";
		if(!empty($_GET['department_sort'])) {
			if($_GET['department_sort'] == '1') $department_order = "t1.id";
			else if($_GET['department_sort'] == '2') $department_order = "t1.mem_group_name, t1.id";
			else if($_GET['department_sort'] == '3') $department_order = "t2.id, t1.id";
		...
		if(sizeof($member_ids)!=0){
			$where .= "AND member_id IN (".implode(',', array_map(function($val){ return sprintf("'%s'", $val); }, $member_ids)).") ";
		}else{
        ...
        if(sizeof(array_filter($loan_ids))){
            $where_loan = " id IN  (".implode(',',array_filter($loan_ids)).") ";
        }
		if(!empty($loan_ids)) {
			$loans = $this->db->query("SELECT t1.id, t1.contract_number, t1.id, t2.loan_type_id  FROM coop_loan as t1
										...
										WHERE {$where_loan} AND t1.loan_status NOT IN ('5')")
								->result_array();
			$loan_members = array_column($loans, 'id');
            $where_loan = " 1=1 ";
            if(sizeof(array_filter($loan_ids))){
                $where_loan = " t2.loan_id IN  (".implode(',',array_filter($loan_ids)).") ";
            }
			$receipts = $this->db->query("SELECT t2.loan_id, t2.period_count FROM coop_receipt as t1
											...
											WHERE {$where_loan} AND t1.finance_month_profile_id = '{$profile_id}'")
											->result_array();
        ...
        if(sizeof(array_filter($loan_atm_ids))){
            $where_loan_atm = " loan_atm_id IN  (".implode(',',array_filter($loan_atm_ids)).") ";
        }
		if(!empty($loan_atm_ids)) {
			$loan_atms = $this->db->query("SELECT t1.loan_atm_id, t1.contract_number, t1.loan_atm_id  FROM coop_loan_atm as t1
										WHERE {$where_loan_atm} AND t1.loan_atm_status NOT IN ('5')")
								->result_array();
			$loan_atm_members = array_column($loan_atms, 'loan_atm_id');
            $where_loan_atm = " 1=1 ";
            if(sizeof(array_filter($loan_atm_ids))){
                $where_loan_atm = " t2.loan_atm_id IN  (".implode(',',array_filter($loan_atm_ids)).") ";
            }
			$receipt_atms = $this->db->query("SELECT t2.loan_atm_id, t2.period_count FROM coop_receipt as t1
											INNER JOIN coop_finance_transaction as t2 ON t1.receipt_id = t2.receipt_id
											WHERE {$where_loan_atm} AND t1.finance_month_profile_id = '{$profile_id}' AND t2.principal_payment > 0")
									->result_array();
	...
	function coop_report_send_deduction_excel(){
		...
		if (!empty($_GET["mem_type"])){
			//if($_GET['dev'] == 'dev'){
			//	echo '<pre>'; print_r($_GET["mem_type"]); echo '</pre>';
			//}
			//if (is_array($_GET["mem_type"]) && !in_array("all", $_GET["mem_type"])){
			//	$member_where .= " AND t4.mem_type_id IN (".implode(',', $_GET["mem_type"]).")";
			//} else if(!is_array($_GET["mem_type"]) && strpos($_GET["mem_type"], "all") === false){
		...
		$finance_month_details = $this->db->query("SELECT * FROM coop_finance_month_detail WHERE {$where_member} AND profile_id = '{$profile_id}'")
											->result_array();
		$finance_member_ids = array_column($finance_month_details, 'member_id');
		...
		if(!empty($loan_atm_ids)) {
			$loan_atms = $this->db->query("SELECT t1.loan_atm_id, t1.contract_number, t1.loan_atm_id  FROM coop_loan_atm as t1 WHERE {$where_loan_atm} AND t1.loan_atm_status NOT IN ('5')")
								->result_array();
			$loan_atm_members = array_column($loan_atms, 'loan_atm_id');
			...
			$receipt_atms = $this->db->query("SELECT t2.loan_atm_id, t2.period_count FROM coop_receipt as t1
											INNER JOIN coop_finance_transaction as t2 ON t1.receipt_id = t2.receipt_id
											WHERE {$where_loan_atm} AND t1.finance_month_profile_id = '{$profile_id}' AND t2.principal_payment > 0")->result_array();
			$loan_atm_receipt_members = array_column($receipt_atms, 'loan_atm_id');
		}
	...
	function check_coop_report_pay_month() {
		...
		$where_member = '1=1 AND member_status <> 3';
		...
		$rs = $this->db->select("t1.member_id")
						->from("(SELECT member_id,profile_id  FROM coop_finance_month_detail WHERE {$where_member} AND profile_id = '".$monthProfile->profile_id."' AND run_status = '1') as t1")
	...
	function coop_report_non_pay_by_member_detail($array_post){
		$arr_data = array();
		//echo '<pre>'; print_r($array_post); echo '</pre>'; exit;
	...
	function coop_report_non_pay_by_member_detail_v4($array_post){
		$arr_data = array();
	...
	function coop_report_non_pay_by_member_detail_excel_v4(){
		$arr_data = array();
		...
		$this->db->join('coop_mem_type AS t6','t3.mem_type_id = t6.mem_type_id','left');
		$this->db->where("t2.non_pay_month = '".$_GET['month']."' AND t2.non_pay_year = '".$_GET['year']."' {$where_data}");
		$this->db->group_by('t1.member_id');
		$this->db->order_by('t5.mem_group_id ASC,t1.member_id ASC');
		$row_data = $this->db->get()->result_array();
	...
	function coop_report_non_pay_by_member_detail_v3($array_post){
		$arr_data = array();
		//echo '<pre>'; print_r($array_post); echo '</pre>'; exit;
		$month = (@$array_post['month'] != '')?$array_post['month']:date('m');
		$year = (@$array_post['year'] != '')?$array_post['year']:(date('Y')+543);
	...
	function coop_report_non_pay_by_member_detail_excel_v3(){
		$arr_data = array();
...
*/
	...
	function coop_report_receive_advance_month_preview() {
		...
		$details = $this->db->select(array(
							...
							->from("(SELECT member_id, profile_id, pay_amount,real_pay_amount FROM coop_finance_month_detail WHERE {$where_member} AND profile_id = '".$datas[0]['profile_id']."') as t8")
							->join("coop_receipt as t9",$receipt_con,"inner")
							->where("t9.receipt_status IS NULL")
							->group_by('t8.member_id , t9.pay_type')
							->get()->result_array();
		if($_GET['dev2'] == 'dev2'){
			echo $this->db->last_query(); exit;
	...
	function coop_report_non_pay_excel(){
		$array_post = $_POST;
		$get_param = '?';
		...
		if($_POST['type_report'] == '0'){
			echo "<script>document.location.href='".base_url(PROJECTPATH.'/report_processor_data/coop_report_non_pay_by_department_excel'.$get_param)."';setTimeout(function(){var ww = window.open(window.location, '_self'); ww.close(); }, 1000);</script>";
			//echo "<script>window.close();</script>";
		}else if($_POST['type_report'] == '1'){
			echo "<script>document.location.href='".base_url(PROJECTPATH.'/report_processor_data/coop_report_non_pay_by_member_detail_excel'.$get_param)."';setTimeout(function(){var ww = window.open(window.location, '_self'); ww.close(); }, 1000);</script>";
		}else if($_POST['type_report'] == '2'){
			echo "<script>document.location.href='".base_url(PROJECTPATH.'/report_processor_data/coop_report_non_pay_by_member_excel'.$get_param)."';setTimeout(function(){var ww = window.open(window.location, '_self'); ww.close(); }, 1000);</script>";
		}
	}
