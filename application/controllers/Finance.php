...
class Finance extends CI_Controller {
	...
	function finance_all_money_report(){
		...
		// ไม่เรียกเก็บเลย
		//----------------
		$where .= " AND mem_type NOT IN (7, 99) AND faction NOT IN (2324,2382,2440,2498)";
		//----------------
		...
		foreach($row_member as $key_member => $row){
			$sum_total = 0;
			 //echo "<pre>";var_dump($deduct_list);echo "</pre>"; exit;
			foreach($deduct_list as $key2 => $value2){
				...
				if($value2['deduct_code']=='OTHER'){
                    ...
                    //echo " LOAN ===> <br><br><br>";
					//echo"<pre>";print_r($LOAN);echo"</pre>"; //exit;
					if(!empty($LOAN)){
						foreach($LOAN as $key3 => $value3){

						     //echo " value3  ==> <br><br>";
							 //echo"<pre>";print_r($value3);echo"</pre><br>";
							 //exit/finance/finance_all_money_report?month=12&year=2562&member_id=00073&run_current=1&limit=50&debug=on;
							...
							if($value3['check_month'] == 1 && $value3['pay_type'] == 'principal' && !empty(array_intersect($refrain_types,[1,3]))){
								...
								//echo $date_last_interest; exit;

                                //Date Target Interest
                                $_date = ($data_get['year']-543)."-".sprintf("%02d",$data_get['month'])."-01";
                                $date_target_interest = date("Y-m-t", strtotime($_date));

                                //ตรวจว่ามีการจ่ายดอกเบี้ยงวดแรกยัง
                                $flag = "";
                                if(!$this->Interest_modal->first_interest_payment($value3['loan_id'])){
                                    $flag = "+1";
                                }
											...
											ROUND(
												...
												IF (
													...
													'.$dd.' * (
														...
														WHERE
															type_id = coop_loan.loan_type
														AND start_date <= "'.$date_target_interest.'"
														ORDER BY
															start_date DESC,
															id DESC
														LIMIT 1
								...
								$query_interest['interest'] = round($query_interest['interest'], 0);
								$period;
								$period_type;
								$pay_type = "";
								if($query_interest['pay_type']==1){ //จ่ายแบบคงต้น
									$refrain_principal = $query_interest['money_period'];
								}else{//จ่ายแบบคงยอด
									$refrain_principal = @$query_interest['money_period'] > $query_interest['interest'] ? @$query_interest['money_period'] - $query_interest['interest'] : 0;
								}
							...
							if($value3['check_month'] == 1 && $value3['pay_type'] == 'interest' && !empty(array_intersect($refrain_types,[2,3]))){
								...
								$data_insert["org_value"] = $query_interest['interest'];
								$data_insert["paid_value"] = 0;
								$data_insert["month"] = $data_get['month'];
								$data_insert["year"] = $data_get['year'];
								$data_insert["status"] = 0;
							...
							if($value3['pay_loan_type'] == '2' && $value3['pay_type'] == 'principal'){
								...
								if(($value3['loan_amount_balance']) >= @$value3['money_period_1']){
									if($value3['check_month'] == 0){
										...
										$yy = date("Y", strtotime("-1 months", strtotime(($data_get['year']-543)."-".sprintf("%02d",$data_get['month'])."-".date('d',strtotime(($data_get['year']-543)."-".$data_get['month']."-01")))) );
										$date_sub = ($data_get['year']-543)."-".sprintf("%02d",$data_get['month'])."-".date('d',strtotime(($data_get['year']-543)."-".$data_get['month']."-01"));
										$date_last_interest =	empty($value3['date_last_interest']) ? date('Y-m-d') : $value3['date_last_interest'];
										$arg = array("member_id" => $row['member_id'], "loan_id" => $value3['loan_id']);
										$interest_rate_last = $this->Interest_modal->get_interest($value3['loan_type_id'], $date_last_interest , $arg);
										$interest_rate_now = $this->Interest_modal->get_interest($value3['loan_type_id'], ($data_get['year']-543)."-".sprintf("%02d",$data_get['month'])."-".date('d',strtotime(($data_get['year']-543)."-".$data_get['month']."-".$data_get['day'])), $arg);
                                        $interest_rate_last = $interest_rate_now;
                                        $_date = ($data_get['year']-543)."-".sprintf("%02d",$data_get['month'])."-01";
                                        $date_target_interest = date("Y-m-t", strtotime($_date));
										...
										// var_dump($query_interest);
										if($query_interest['pay_type']==1){ //จ่ายแบบคงต้น
											$data_arr[$p]['pay_amount'] = round($query_interest['money_period']);
										}else{//จ่ายแบบคงยอด
											$in = round($query_interest['interest']);
											$data_arr[$p]['pay_amount'] = @$query_interest['money_period'] > $in ? @$query_interest['money_period'] - $in : 0;
										}
								...
								if($value3['check_month'] == 0 && $value3['pay_type'] != 'principal'){
									...
									$yy = date("Y", strtotime("-1 months", strtotime(($data_get['year']-543)."-".sprintf("%02d",$data_get['month'])."-".date('d',strtotime(($data_get['year']-543)."-".$data_get['month']."-01")))) );
									$date_sub = ($data_get['year']-543)."-".sprintf("%02d",$data_get['month'])."-".date('d',strtotime(($data_get['year']-543)."-".$data_get['month']."-01"));
									$date_last_interest = empty($value3['date_last_interest']) ? ($data_get['year']-543)."-".sprintf("%02d",$data_get['month'])."-".date('d',strtotime(($data_get['year']-543)."-".$data_get['month']."-".$data_get['day'])) : $value3['date_last_interest'];
									$arg = array("member_id" => $row['member_id'], "loan_id" => $value3['loan_id']);
									$interest_rate_last = $this->Interest_modal->get_interest($value3['loan_type_id'], $date_last_interest , $arg);
									$interest_rate_now = $this->Interest_modal->get_interest($value3['loan_type_id'], ($data_get['year']-543)."-".sprintf("%02d",$data_get['month'])."-".date('d',strtotime(($data_get['year']-543)."-".$data_get['month']."-".$data_get['day'])), $arg);
                                    //$interest_rate_last = $interest_rate_now;
									...
									// echo "<hr>";
									// exit;
									$data_arr[$p]['pay_amount'] = round($query_interest['interest']);
									// $data_arr[$p]['pay_amount'] = round($query_interest['interest'], 2);
									// echo "<br>!!!!!!!!!!!!!!!!!!!!!!!!".$data_arr[$p]['pay_amount'];
									// var_dump($data_arr);
								}else{
					...
					//ทุนเรือนหุ้น
					if(in_array(@$row['apply_type_id'], [1,2]) && !in_array($row['mem_type'], [4]) && $check_refrain_share == 0 && @$row['share_month'] > 0){
					...
					if($value_process['real_pay_amount']>0){
						...
						if($value_process['deduct_code'] == 'LOAN' || $value_process['deduct_code'] == 'GUARANTEE'){
							...
							if($value_process['pay_type'] == 'principal'){
								...
								$data_insert = array();
								$data_insert['date_last_interest'] = $process_date;
