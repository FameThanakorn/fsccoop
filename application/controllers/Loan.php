...
////////////////////////////////////////////////////////
	...
	public function loan_approve()
	{
		...
		$this->paginater_all->select('coop_loan.*, IF(coop_loan.loan_type_input	= "1","ระบบกู้เงินออนไลน์",coop_user.user_name) AS user_name, coop_loan_type.petition_file');
	...
	function loan_approve_save(){
		// $this->db->trans_start();
			...
			$data_insert['date_last_interest'] = date("Y-m-t",strtotime($date_approve_time));
			$this->db->where('id', @$_GET['loan_id']);
			$this->db->update('coop_loan', $data_insert);
...
//		@$data['coop_loan_cost']['other'] = @$rs[0]['other'];
	...
	function petition_normal_pdf($loan_id=0){
		$arr_data = array();
