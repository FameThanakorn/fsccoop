...
class Manage_member_share extends CI_Controller {
	...
	function approve_register(){
		...
		$this->paginater_all->select('*');
		$this->paginater_all->main_table('coop_mem_apply');
		$this->paginater_all->where("member_status IN ('3','4')");
		$this->paginater_all->page_now(@$_GET["page"]);
		$this->paginater_all->per_page(20);
		$this->paginater_all->page_link_limit(20);
		...
		if(!empty($_POST['checkbox'])){
			foreach($_POST['checkbox'] as $key => $value){
				...
				if($_POST['status_to']=='1'){
					...
					$this->db->from('coop_mem_apply');
					$this->db->where("coop_mem_apply.id = '".$value."'");
					...
					$this->db->select('member_id');
					$this->db->from('coop_mem_apply');
					//$this->db->where("apply_type_id = '".$row_member['apply_type_id']."' AND SUBSTRING(member_id, 1, 2) = '".$prefix."'");
					//$this->db->where("apply_type_id IN ('".implode(',',$arr_apply_type)."') AND SUBSTRING(member_id, 1, 2) = '".$prefix."'");
					$this->db->where("member_id LIKE '".$prefix."%'");
					$this->db->order_by('member_id DESC');
					$this->db->limit(1);
					$row = $this->db->get()->result_array();
					//echo $this->db->last_query(); echo '<br>';
				...
				if($_POST['status_to']=='1'){
					...
					$this->db->order_by('card_number DESC');
					$this->db->limit(1);
					$row = $this->db->get()->result_array();
					if(!empty($row)) {
						$id = (int)$row[0]["card_number"];
						$card_number = sprintf("%06d", $id + 1);
					}else {
						$card_number = "000001";
					}
					$atm_number = $this->random_atm_number();
					$data_insert_atm_card = array();
					$data_insert_atm_card['member_id'] = $member_id;
					$data_insert_atm_card['card_number'] = $card_number;
					$data_insert_atm_card['atm_number'] = $atm_number;
					$data_insert_atm_card['atm_card_status'] = '0';
					$this->db->insert('coop_atm_card', $data_insert_atm_card);*/
	...
	function auto_create_coop_account($member_id){
		...
		if(empty($rs_account)){
			...
			foreach($rs_auto_create as $key => $row_auto_create ){
				...
				$this->db->select('*');
				$this->db->from('coop_mem_apply');
				$this->db->where("member_id = '".$member_id."' AND apply_type_id in (1,3,6)");
				$rs_member = $this->db->get()->result_array();
				$row_member = @$rs_member[0];
				if(!empty($row_member)){
					$data_insert = array();
	...
	function search_from_id_card(){
		...
		$this->db->select(array('*'));
		$this->db->from('coop_mem_apply');
		$this->db->where("id_card = '".$_POST['id_card']."'");
	...
	function member_loan(){
		...
		if($member_id != '') {			
			...
			foreach($rs_loan AS $key=>$row_loan){
				...
				$arr_loan[$key]['period_count'] = (@$row_finance_transaction['period_count'] != '')?@$row_finance_transaction['period_count']:'0';//งวดที่
		...
		if($member_id != ''){
			...
			if(@$_GET['month'] == '' && @$_GET['year'] == ''){
				$profile_month = @$rs_profile_last['profile_month'];
				$profile_year = @$rs_profile_last['profile_year'];				
			}else{
			...
			if(!empty($arr_profile_id)){
				...
				$this->db->join("coop_receipt","coop_receipt.finance_month_profile_id = coop_finance_month_detail.profile_id  AND coop_receipt.member_id = coop_finance_month_detail.member_id","left");
			...
			/*if(!empty($arr_profile_id)){
				...
				foreach($rs_finance_detail AS $key_detail=>$value_detail){
					...
					foreach($rs_pay_receipt AS $key_receipt=>$row_pay_receipt){
						$row_finance_detail[$key_detail]['receipt_id'][] = @$row_pay_receipt;
					}
				}
			}
			*/
			...
			if(!empty($rs)){
				foreach(@$rs as $key => $row){
					...
					if($row['deduct_code'] == 'LOAN'){
						$deduct_detail = $text_pay_loan_type.'  '.$row['contract_number'];
