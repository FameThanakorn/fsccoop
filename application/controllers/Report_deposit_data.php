		...
		if($_GET["report_type"] == 1) {
			...
			$join_arr[$x]['table'] = "(
				...
				(
					...
					WHERE
						transaction_time <= '{$start_date} 23:59:59'
					AND coop_maco_account.account_id = coop_account_transaction.account_id
					ORDER BY
						transaction_time DESC, transaction_id DESC
					LIMIT 1
			...
			$join_arr[$x]['condition'] = "t1.mem_id = t3.member_id";
			$join_arr[$x]['type'] = 'left';
			$x++;
			$join_arr[$x]['table'] = 'coop_prename as t4';
			$join_arr[$x]['condition'] = "t3.prename_id = t4.prename_id";
			$join_arr[$x]['type'] = 'left';

			$this->paginater_all->field_count("t1.account_id");
			$this->paginater_all->type(DB_TYPE);

			$this->paginater_all->select('t1.account_id, t3.member_id, t2.transaction_balance, t3.firstname_th, t3.lastname_th, t4.prename_short');
			...
			$this->paginater_all->order_by('t1.account_id');
			...
			if (intval($row['page']) == ceil($row['num_rows']/$row['per_page'])) {
				$rs = $this->db->select("SUM(t2.transaction_balance) as sum_transaction_balance")
