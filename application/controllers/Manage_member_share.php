...
class Manage_member_share extends CI_Controller {
	...
	function save_add(){
		...
		//echo '<pre>'; print_r($data_historys); echo '</pre>';
		...
		echo"<script> document.location.href='".PROJECTPATH."/manage_member_share/add/{$id}' </script>";
		exit;
	}
		...
		if(!empty($_POST['checkbox'])){
			...
			foreach($_POST['checkbox'] as $key => $value){
				...
				if($_POST['status_to']=='1'){
                    ...
                    //echo "<pre>"; print_r($tmp_prefix); exit;
                    $where = " AND 1=1";
					if(empty($prefix)) {
					...
					//check mem_type_code
					if(empty($prefix)) {
						$where_check_mem = "mem_type_code = '".$prefix."' OR mem_type_code IS NULL";
						$where_m = "";
					}else{	
						$where_check_mem = "mem_type_code = '".$prefix."'";
						$where_m = " AND member_id LIKE '" . $prefix . "%' ";
					}
					$this->db->select('mem_type_id,mem_type_code');
					$this->db->from('coop_mem_type');
					$this->db->where($where_check_mem);
					$check_mem_type = $this->db->get()->result_array();
					$arr_mem_type = array_column($check_mem_type, 'mem_type_id');

					$this->db->select('member_id');
					$this->db->from('coop_mem_apply');
					$this->db->where("mem_type_id IN ('".implode("','",$arr_mem_type)."')".$where_m.$where);
					$this->db->order_by('member_id DESC');
					$this->db->limit(1);
					$row = $this->db->get()->result_array();
	...
	function check_salary_and_share(){
		...
		$this->db->select(array('percent_salary','share_monthly_min'));
