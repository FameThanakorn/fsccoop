...
class Loan_libraries extends CI_Model {
	...
	public function cal_atm_interest_report_test($data,$return_type='echo',$type_count_date="",$is_process=true, $is_counter=false){
		...
		$loan_detail = @$this->db->get_where("coop_loan_atm", array(
			"loan_atm_id" => $data['loan_atm_id']
		))->result_array()[0];
		...
		if($is_process){
			...
			// $fix_date_end.= " AND receipt_id is not null ORDER BY transaction_datetime desc, loan_atm_transaction_id desc limit 1";
		}
		...
		$tmp_interest_deduct		= 0;
		$sum_collect_interest		= 0;
		$sum_collect_interest_c		= array();
		$receipt_c					= array("10"=>false, "11"=>false, "12"=>false, "01"=>false);
		$payment_status				= array("10"=>false, "11"=>false, "12"=>false, "01"=>false);
		$oldest_date				= "";
		$is_settlement				= false;
		$sum_un_interest			= 0;
		...
		if($query){
			...
			$oldest_date = @$query[0]['transaction_datetime'];
			...
			foreach ($query as $key => $value) {
				...
				if( date("m", strtotime( explode(" ", $value['transaction_datetime'])[0] ) ) != date("m", strtotime($last_atm_transaction_date) ) ){
					...
					if( strtotime($last_atm_transaction_date_part) >= strtotime("2018-10-01 00:00:00") && strtotime($last_atm_transaction_date_part) <= strtotime("2019-01-31 23:59:59")){
						// $sum_real_payment_interest		= 0;
						// echo "|".$last_atm_transaction_date_part."|";
						
						$sum_collect_interest		+= $last_bf * $interest_rate_atm / 100 * $date_count / $days_in_year;
						if(strtotime($last_atm_transaction_date_part) > strtotime( date("Y-m-t", strtotime($loan_detail['approve_date']))." 23:59:59") ){
							$sum_collect_interest_c[@date("m", strtotime($last_atm_transaction_date_part))]	+= $last_bf * $interest_rate_atm / 100 * $date_count / $days_in_year;
						}else{
							$sum_un_interest += $last_bf * $interest_rate_atm / 100 * $date_count / $days_in_year;
						}
						// echo $last_atm_transaction_date_part ." | ". date("Y-m-t", strtotime($loan_detail['approve_date']))." ";
						// echo @date("m", strtotime($last_atm_transaction_date_part))."|".($last_bf * $interest_rate_atm / 100 * $date_count / $days_in_year)."|".$sum_collect_interest_c[@date("m", strtotime($last_atm_transaction_date_part))];
					}else{
						$mm = date("m", strtotime($last_atm_transaction_date_part) );
						$yy = date("Y", strtotime($last_atm_transaction_date_part) );
	
						$this->db->where("loan_atm_id", $data['loan_atm_id']);
						$this->db->where("coop_finance_month_profile.profile_month", $mm);
						$this->db->where("coop_finance_month_profile.profile_year", ($yy+543));
						$this->db->join("coop_finance_month_profile", "coop_finance_month_profile.profile_id = coop_finance_month_detail.profile_id");
						$ch_finance_month_detail = $this->db->get("coop_finance_month_detail")->result_array();
	
						if(!$ch_finance_month_detail && strtotime($last_atm_transaction_date_part) < strtotime("2018-10-01")){
							...
							$receipt_detail = @$this->db->get_where("coop_finance_transaction", array(
								...
								"loan_atm_id" => $data['loan_atm_id'],
								"payment_date" => date("Y-m-d", strtotime($last_atm_transaction_date_part))
							))->result_array();	
					...
					if(@$_GET['excel']){
						...
						",".($sum_of_atm + ($value['receipt_id']=="" && $sum_of_atm!=0 ? $withdraw_atm : 0)).",".$withdraw_atm.",".$sum_collect_interest;
						fputcsv($fp, explode(",", $str));
					}
					...
					// $date_count 					= $diff->format("%a");
					$diff 							= date_diff(date_create($last_atm_transaction_date_part), date_create(date("Y-m-d", strtotime($value['transaction_datetime']))));
					$date_count 					= $diff->format("%a");
				...
				if($is_counter && strtotime($value['transaction_datetime']) <= strtotime($date_calc_interest_counter) ){
					// $tmp_interest_deduct += $last_bf * $interest_rate_atm / 100 * $date_count / $days_in_year;
					$sum_of_atm = 0;
				}
				...
				if($date_count > 31){
					if( explode(" ", $date_calc_diff_day)[0] > $row_atm_setting[0]['start_date'] && explode(" ", $query[$key-1]['transaction_datetime'])[0] < $row_atm_setting[0]['start_date']){
						$this->db->select('interest_rate, start_date');
						$this->db->from('coop_loan_atm_setting_template');
						$this->db->where("start_date < '".$row_atm_setting[0]['start_date']."'");
						$this->db->order_by("start_date DESC,run_id DESC");
						$this->db->limit(1);
						$row_atm_setting_over = $this->db->get()->result_array();
						$interest_rate_atm_over = $row_atm_setting_over[0]['interest_rate'];

						$diff_over						= date_diff(date_create($last_atm_transaction_date_part), date_create(date("Y-m-d", strtotime($row_atm_setting[0]['start_date']))));
						$date_count_over_part1 			= $diff_over->format("%a");
						$date_count						= $date_count - $date_count_over_part1;

						$temp_interest 					+= $last_bf * $interest_rate_atm_over / 100 * $date_count_over_part1 / $days_in_year;
						$sum_of_interest 				+= $temp_interest;
						$remain_interest 				+= $temp_interest;

						if($is_atm==true && strtotime($last_atm_transaction_date_part) > strtotime($date_s_calc_atm)){
							$sum_of_interest_atm += round(($sum_of_atm) * $interest_rate_atm_over / 100 * $date_count_over_part1 / $days_in_year, 2);
							if( strtotime($last_atm_transaction_date_part) <= strtotime($_month_1) )
								$sum_of_settlement_interest += round(($sum_of_atm_settlement) * $interest_rate_atm_over / 100 * $date_count_over_part1 / $days_in_year, 2);
						}

						$tmp_sum_pay_interest = 0;
						$str = "";
						if( strtotime($row_atm_setting[0]['start_date']) >= strtotime($row_atm_setting[0]['start_date']) ){
							// $sum_real_payment_interest		= 0;

							$date1 = new DateTime($last_atm_transaction_date_part);
							$date2 = new DateTime($row_atm_setting[0]['start_date']);
		
							$diff = $date1->diff($date2);
							$diff_month = $diff->format('%m');
							for($i = 1; $i <= $diff_month; $i++){
								
								$m_date = date("m", strtotime($last_atm_transaction_date_part))+$i;
								if($m_date > 12){
									$m_date = 1;
								}

								$mm = date("m", strtotime("+".$i." months", strtotime($last_atm_transaction_date_part)) );
								$yy = date("Y", strtotime("+".$i." months", strtotime($last_atm_transaction_date_part)) );

								$this->db->where('pay_type', 'interest');
								$this->db->where("loan_atm_id", $data['loan_atm_id']);
								$this->db->where("coop_finance_month_profile.profile_month", $m_date);
								$this->db->where("coop_finance_month_profile.profile_year", ($yy+543));
								$this->db->join("coop_finance_month_profile", "coop_finance_month_profile.profile_id = coop_finance_month_detail.profile_id");
								$ch_finance_month_detail = $this->db->get("coop_finance_month_detail")->result_array()[0];
								// echo $this->db->last_query().";";
								if($ch_finance_month_detail){
									// var_dump($ch_finance_month_detail['pay_amount']);
									$str .= $m_date." | ";
									$tmp_sum_pay_interest += ($ch_finance_month_detail['pay_amount']);
									$sum_collect_interest_c[$m_date]	+= ($ch_finance_month_detail['pay_amount']);
								}

								if( strtotime(date("Y-m-t", strtotime($yy."-".$m_date."-05") )) >= strtotime("2018-10-01") && strtotime(date("Y-m-t", strtotime($yy."-".$m_date."-05") )) <= strtotime("2019-01-31") ){
									$tmp_diff_over					= date_diff(date_create( date("Y-m-t", strtotime($yy."-".$m_date."-05") ) ), date_create( date("Y-m-t", strtotime("+1 months", strtotime($yy."-".$m_date."-05")) ) ));
									$tmp_diff 						= $tmp_diff_over->format("%a");
									$sum_collect_interest			+= round($last_bf * $interest_rate_atm_over / 100 * ($tmp_diff) / $days_in_year,0);
								}
							...
							// $sum_collect_interest			+= $last_bf * $interest_rate_atm_over / 100 * ($date_count_over_part1-1) / $days_in_year;
							// $sum_collect_interest_c[@date("m", strtotime("-1 months", strtotime(explode(" ", $row_atm_setting[0]['start_date'])[0])) )]	+= $tmp_sum_pay_interest;
							// echo @date("m", strtotime("-1 months", strtotime(explode(" ", $row_atm_setting[0]['start_date'])[0])) )." | ".$sum_collect_interest_c[date("m", strtotime("-1 months", strtotime(explode(" ", $row_atm_setting[0]['start_date'])[0])) )]." | ".$tmp_sum_pay_interest." ";
							
							// if( (round($last_bf * $interest_rate_atm_over / 100 * ($date_count_over_part1-1) / $days_in_year,2) - $tmp_sum_pay_interest) == 1 ){
							// 	$sum_collect_interest_c[@date("m", strtotime($last_atm_transaction_date_part))] += 999;
							// }else if( (round($last_bf * $interest_rate_atm_over / 100 * ($date_count_over_part1-1) / $days_in_year,2) - $tmp_sum_pay_interest) == -1 ){
							// 	$sum_collect_interest_c[@date("m", strtotime($last_atm_transaction_date_part))] -= 9999;
							// }
						}
						...
						if(@$_GET['excel']){
							...
							$str = date("Y-m-t", strtotime("-1 months", strtotime(explode(" ", $row_atm_setting[0]['start_date'])[0])) ).",".$bf.",".
								$detail.",".
								(@$value['receipt_id']!="" ? @$receipt_detail[0]['principal'] : $withdraw_atm).",".
								(@$value['receipt_id']!="" ? @$receipt_detail[0]['interest'] : "0").",".
								$interest_rate_atm_over."%".",".
								($date_count_over_part1-1).",".
								"=".$last_bf ."*". $interest_rate_atm_over ."/ 100 * ". ($date_count_over_part1-1) ."/". $days_in_year.",".
								"=".($is_atm==true ? ($sum_of_atm) ."*". $interest_rate_atm_over ."/ 100 *". ($date_count_over_part1-1) ."/". $days_in_year : "0" ).",".($sum_of_atm + ($value['receipt_id']=="" && $sum_of_atm!=0 ? $withdraw_atm : 0)).",".$withdraw_atm.",".$sum_collect_interest.",** ดอกเบี้ยออกเรียกเก็บเดือน $str จำนวน ".$tmp_sum_pay_interest;
							fputcsv($fp, explode(",", $str));
						}
					}
				...
				if($is_atm==true && strtotime($last_atm_transaction_date) > strtotime($date_s_calc_atm)){
					$sum_of_interest_atm += round(($sum_of_atm) * $interest_rate_atm / 100 * $date_count / $days_in_year, 2);
					if( strtotime($last_atm_transaction_date) >= strtotime($_month_1) )
						$sum_of_settlement_interest += round(($sum_of_atm_settlement) * $interest_rate_atm / 100 * $date_count / $days_in_year, 2);
				}

				//ดอกเบี้ย หน้าเค้าเตอร์ และหักกลบ
				...
				if(@$_GET['debug']){
					...
					echo  $bf." * ".$interest_rate_atm." / 100 * ".$date_count." / $days_in_year|<br>";
					echo "collect_interest: $collect_interest<br>";
					echo "settlement_interest: $sum_of_settlement_interest ($sum_of_atm_settlement) $_month_1<br>";
				}
				if(@$_GET['excel']){
					$this->db->select(array(
						"sum(principal_payment) as principal", 
				...
				//ทำรายการกู้ATM
				if($is_counter && $value['receipt_id']=="" && $withdraw_atm > 0 && strtotime($last_atm_transaction_date) >= strtotime($date_latest_process)){
					$is_atm 			= true;
					$sum_of_atm 		+= $withdraw_atm;
					$sum_of_atm_settlement += $withdraw_atm;
				}else if($value['receipt_id']=="" && $withdraw_atm > 0 && strtotime($last_atm_transaction_date) >= strtotime($date_s_calc_atm)){
					$is_atm 			= true;
					$sum_of_atm 		+= $withdraw_atm;
					$sum_of_atm_settlement += $withdraw_atm;
				}
		...
		if($is_process){
			...
			// $interest = $tmp_interest_deduct;
			// $sum_of_settlement_interest += ($bf<=0 ? 0 : $bf) * $date_count * $interest_rate_atm / 100 / $days_in_year;
		}
		...
		$unpayment = 0;
		// echo $oldest_date." ";
		foreach ($receipt_c as $key => $value) {
			// var_dump((string)$key);
			// echo "<br><hr>";
			if($value===true){
				$sum_c += $sum_collect_interest_c[(string)$key];
			}

			// if($payment_status[(string)$key] == false && $oldest_date < '2018-10-01'){
			if($payment_status[(string)$key] == false && ($sum_real_payment_interest - $sum_collect_interest) < 0 ){
				$unpayment += $sum_collect_interest_c[(string)$key];
				// echo $unpayment;
			}
		}

		if($is_settlement || strtotime($loan_detail['approve_date']) >= strtotime("2019-01-01 00:00:00")){
			$sum_c 						= 0;
			$unpayment 					= 0;
			$sum_real_payment_interest	= 0;
			$sum_collect_interest		= 0;
		...
		// echo $oldest_date."{".$sum_collect_interest."}";
		// var_dump($sum_collect_interest_c);
		// echo "<br>";
		// var_dump($receipt_c);
		// echo "<br>";
		...
		$arr_loan_interest['sum_collect_interest'] = round($sum_collect_interest - $sum_c - $unpayment, 2);
		$arr_loan_interest['settlement_interest'] = round($sum_of_settlement_interest, 0);
		...
		if($is_counter){
			...
			if($sum_of_interest_atm <= 0){
				$arr_loan_interest['interest_month'] = 0;
			}else{
				$arr_loan_interest['interest_month'] = round($total_interest + $sum_of_interest_atm);
			}
			// if($sum_of_settlement_interest <= 0){
			// 	$arr_loan_interest['interest_month'] = 0;
			// }else{
			// 	$arr_loan_interest['interest_month'] = round($total_interest + $sum_of_settlement_interest);
			// }
		}else if($rs_chk){//หน้าเค้าเตอร์
		...
		if(@$_GET['excel']){
			...
			fputcsv($fp, explode(",", $str));
			// $str = "ดอกเบี้ยที่จ่ายจริง,,".round($sum_real_payment_interest, 2);
			// fputcsv($fp, explode(",", $str));
			// $str = "ดอกเบี้ยจากการคำนวณ,,".round($sum_collect_interest, 2);
			// fputcsv($fp, explode(",", $str));
			// $str = "เงินคืน ต.ค. ถึง มกราคม,,=(".round($sum_real_payment_interest, 2)."-".round($sum_collect_interest, 2)."+".round($sum_c, 2)."+".round($unpayment, 2)."+".round($sum_un_interest,2).")";
			// fputcsv($fp, explode(",", $str));
			fclose($fp);
		}
