...
class Loan_libraries extends CI_Model {
    ...
    public function get_contract_number($year, $loan_type){
	...
	public function cal_atm_interest_report_test($data,$return_type='echo',$type_count_date="",$is_process=true, $is_counter=false){
		...
		$date_calc_interest_atm_after_payment = date("Y-m-", strtotime("-1 months", strtotime($data['date_interesting'])))."01";
		...
		if($query){
			...
			foreach ($query as $key => $value) {
				...
				if($is_atm==true && strtotime($last_atm_transaction_date) > strtotime($date_calc_interest_atm_after_payment)){
				...
				if($is_atm_after_process == true){
					$sum_of_atm_after_process += round(($sum_of_atm) * $interest_rate_atm / 100 * $date_count / $days_in_year, 2);
				}
				//ดอกเบี้ย หน้าเค้าเตอร์ และหักกลบ
				//skip การคิดดอกเบี้ยจากหนี้คงเหลือ ไปจนถึงสิ้นเดือน เพราะมีการคิดดอกเบี้ยไปจนถึงสิ้นเดือนแล้ว
				if(($is_counter && strtotime($value['transaction_datetime']) > strtotime( date("Y-m-t", strtotime($date_latest_process))." 23:59:59" ))){
		...
		if($rs_chk || $is_counter){
			...
			if($is_counter)
				$arr_loan_interest['interest_month'] = round($total_interest + $sum_of_settlement_interest + $sum_of_atm_after_process);
			else
    ...
    public function calc_interest_loan_type($loan_amount, $loan_type, $date1, $date2){
        ...
        if($diff_y > 1){
            for ($i=0; $i < $diff_y; $i++) {
                ...
                if($diff_y-1 == $i){
                    $query = $this->db->query("select calc_loan_interest($loan_amount, $loan_type, '$c_date1', '$date2' )");
                }else{
                    $c_date2 = date("Y", strtotime($c_date1) ) . "-12-" . date("t", strtotime($c_date1) );
                    $query = $this->db->query("select calc_loan_interest($loan_amount, $loan_type, '$c_date1', '$c_date2' )");
                }
