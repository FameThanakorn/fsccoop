<div class="layout-content">
    <div class="layout-content-body">
		...
		<h1 style="margin-bottom: 0">รายงานเงินเก็บได้บางส่วน</h1>
		<?php $this->load->view('breadcrumb'); ?>
		<div class="row gutter-xs">
			<div class="col-xs-12 col-md-12">
				<div class="panel panel-body" style="padding-top:0px !important;">
				<form action="<?php echo base_url(PROJECTPATH.'/report_processor_pay_some_data/coop_report_pay_some_preview'); ?>" id="form1" method="POST" target="_blank">
					<h3></h3>
					...
					<div class="form-group g24-col-sm-24">
						...
						<div class="g24-col-sm-7">
							<input type="button" class="btn btn-primary" style="width:100%" value="รายงานเงินเก็บได้บางส่วน" onclick="check_empty();">
						</div>
...
<script>	
	...
	function check_empty(){
		if ($("#type_report").val() == 1 || $("#type_report").val() == 2) {
			...
			$.ajax({
				url: base_url+'/report_processor_pay_some_data/check_coop_pay_some',
				method:"post",
				data:{ 
					month: $('#month').val(),
					year: $('#year').val(),
				...
				success:function(data){
					...
					$.unblockUI();
					if(data == 'success'){
						link_to =  base_url+'report_processor_pay_some_data/coop_report_pay_some_preview';
	...
	function export_excel(){
		...
		if($('#type_report').val() == ''){
			...
			link_to =  base_url+'report_processor_pay_some_data/coop_report_pay_some_excel';
			$('#form1').attr('action', link_to);
