		...
		<div class="row gutter-xs">
			<div class="col-xs-12 col-md-12">
				<div class="panel panel-body">
					...
					<form action="<?=base_url('loan/save_manage_installment')?>" method="post" name="myForm" id="myForm" onsubmit="event.preventDefault(); validateMyForm();">
						<div class="g24-col-sm-24" style="margin-bottom: 15px;">
							...
							<div class="form-group g24-col-sm-8">
								<label class="g24-col-sm-8 control-label" for="form-control-2">เลขที่สัญญา</label>
								<div class="g24-col-sm-14">
									<div class="input-group">
										<input id="contract_number" class="form-control" type="text" value="" required onkeypress="return runScript(event)"
										 autocomplete="off">
						...
						<div class="g24-col-sm-24" style="margin-bottom: 30px;">
							...
							<div class="form-group g24-col-sm-16">
								<label class="g24-col-sm-4 control-label" for="form-control-2">ประเภทการการชำระเงิน</label>
								<div class="g24-col-sm-6">
									<select name="old_installment_period_type" id="old_installment_period_type" class="form-control" disabled="">
						...
						<div class="g24-col-sm-24" style="margin-bottom: 30px;">
							<div class="form-group g24-col-sm-8">
								<label class="g24-col-sm-10 control-label " for="form-control-2">ผ่อนชำระต่อเดือน</label>
								<div class="g24-col-sm-14">
							...
							<div class="form-group g24-col-sm-16">
								<label class="g24-col-sm-4 control-label " for="form-control-2">จำนวนงวด</label>
								<div class="g24-col-sm-6">
						...
						<div class="g24-col-sm-24" style="margin-bottom: 30px;">
							<div class="form-group g24-col-sm-8">
								<label class="g24-col-sm-10 control-label" for="form-control-2">รูปแบบการชำระ</label>
								<div class="g24-col-sm-14">
									<select name="new_installment_type" id="new_installment_type" class="form-control">
										<option value="1">ต้นเท่ากัน</option>
										<option value="2">ยอดเท่ากัน</option>
									</select>
								</div>
							...
							<div class="form-group g24-col-sm-16">
								<label class="g24-col-sm-4 control-label" for="form-control-2">ประเภทการการชำระเงิน</label>
								<div class="g24-col-sm-6">
									<select name="new_installment_period_type" id="new_installment_period_type" class="form-control">
										<option value="1">งวดที่ต้องการผ่อน</option>
										<option value="2">เงินที่ต้องการผ่อนต่องวด</option>
									</select>
								</div>
						...
						<div class="g24-col-sm-24" style="margin-bottom: 30px;">
							<div class="form-group g24-col-sm-8">
								<label class="g24-col-sm-10 control-label " for="form-control-2">ผ่อนชำระต่อเดือน</label>
								<div class="g24-col-sm-14">
									<input class="form-control " type="text" value="" id="new_period" name="new_period_per_month" onkeyup='format_number_1(event, this);check_period();' onchange='format_number_2(event, this)' readonly>
								</div>
							</div>
							...
							<div class="form-group g24-col-sm-16">
								<label class="g24-col-sm-4 control-label " for="form-control-2">จำนวนงวด</label>
								<div class="g24-col-sm-6">
									<input class="form-control " type="text" value="" id="new_payment" name="new_period_amount"  onkeyup="format_number_1(event, this);check_period();">
								</div>
							</div>
						</div>
						...
						<div class="row" style="margin-top: 15px;margin-bottom: 15px;">
							<div class="col-md-offset-4 col-md-4 text-center">
								...
								<button class="btn btn-primary" id="submit_button" type="button">บันทึก</button>
							</div>
						</div>
...
<script>
	...
	$( "#new_installment_period_type" ).change(function() {
		console.log($(this).val());
		if($(this).val()==1){
			$('#new_period').prop('readOnly', true);
			$('#new_payment').prop('readOnly', false);
		}else{
			$('#new_period').prop('readOnly', false);
			$('#new_payment').prop('readOnly', true);
		}
	});
        ...
        $.ajax({
				...
				success: function (data) {
					...
					if (data == "FALSE") {
						...
						$('#old_installment_period_type option[value='+loan['period_type']+']').attr('selected','selected');
	...
	function goto() {
		...
		window.open('<?=base_url('loan/manage_installment_print/')?>' + loan_id, '_blank');
	}
	...
	//type 1= alert
	function check_period(type='') {
		var  new_installment_type  = $('#new_installment_type').val();
		var  new_installment_period_type  = $('#new_installment_period_type').val();
		var  new_period_per_month  = parseFloat(removeCommas($('#new_period').val()));
		var  new_period_amount  = parseFloat(removeCommas($('#new_payment').val()));
		var  loan_id  = $('#loan_id').val();
		if(loan_id == ''){
			swal('กรุณาเลือกเลขที่สัญญา' , '' , 'warning');
		}else{
			$.post(base_url+"loan/get_period", 
			{	
				new_installment_type:new_installment_type,
				new_installment_period_type:new_installment_period_type,
				new_period_per_month:new_period_per_month,
			...
			, function(result){				
				...
				//console.log(obj);
				if(new_installment_period_type == 2){
					$('#new_payment').val(obj.period_amount);	
				}
				
				if(new_installment_period_type == 1){
					$('#new_period').val(obj.money_per_period);	
				}

				if(type == '1'){
					if(obj.text != ''){
						swal('' , obj.text , 'warning');
					}else{
						$('#myForm').submit();
					}
				}	
