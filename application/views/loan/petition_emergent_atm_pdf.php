...
function num_format($text) {
	...
	$pdf->AddPage();
		...
		if($pageNo == '1'){
			...
			$pdf->SetXY( 130, $y_point );
			$pdf->MultiCell(60, 5, U2T($this->center_function->convert($data['loan_amount'])), $border, 'C');
			
			/*$y_point = 137;
			$pdf->SetXY( 120, $y_point );
			$pdf->MultiCell(80, 5, U2T($data['loan_reason']), $border, 1);*/
			...
			/*$y_point = 203;
			$pdf->SetXY( 50, $y_point );
			$pdf->MultiCell(70, 5, U2T($data['prename_short'].$data['firstname_th']." ".$data['lastname_th']), $border, 1);
			$pdf->SetXY( 137, $y_point );
			$pdf->MultiCell(42, 5, U2T(num_format($data['loan_amount'])), $border, 'R');
			
			$y_point = 210;
			$pdf->SetXY( 23, $y_point );
			$pdf->MultiCell(69, 5, U2T($this->center_function->convert($data['loan_amount'])), $border, 'C');
			$pdf->SetXY( 137, $y_point );
			$pdf->MultiCell(69, 5, U2T($this->center_function->ConvertToThaiDate($data['date_transfer'],0,1)), $border, 1);
			
			$y_point = 227;
			$pdf->SetXY( 130, $y_point );
			$pdf->MultiCell(42, 5, U2T($data['firstname_th']." ".$data['lastname_th']), $border, 'C');*/
		}
	}
