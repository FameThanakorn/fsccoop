...
function num_format($text) {
	...
	$filename = $_SERVER["DOCUMENT_ROOT"]."/fsccoop"."/assets/document/loan_education" ;
	...
	$pdf->AddPage();
		...
		if($pageNo == '1'){
			$y_point = 25.5; //เลขที่รับ
			$pdf->SetXY( 33, $y_point );
			$pdf->MultiCell(35, 5, U2T($data['contract_number']), $border, 'C');
			$pdf->SetXY( 80.5, $y_point );
			$pdf->MultiCell(8, 5, U2T($data['contract_number']), $border, 'C');
			$pdf->SetXY( 97.5, $y_point );
			$pdf->MultiCell(17, 5, U2T($data['contract_number']), $border, 'C');
			
			// $y_point = 17;
			// $pdf->SetXY( 21, $y_point );
			// $pdf->MultiCell(30, 5, U2T($this->center_function->ConvertToThaiDate($data['createdatetime'],0,0)), $border, 1); //วันที่ -- เลขที่รับ

			// $y_point = 24;
			// $pdf->SetXY( 15, $y_point );
			// $pdf->MultiCell(22, 5, U2T(''), $border, 'C'); // เจ้าหน้าที่ผู้รับ

			// $y_point = 150;
			// $pdf->SetXY( 150, $y_point );
			// $pdf->MultiCell(10, 5, U2T(''), $border, 'C'); // เจ้าหน้าที่ผู้รับ
		...
		$filename = $_SERVER["DOCUMENT_ROOT"]."/fsccoop"."/assets/document/loan_education.pdf" ;
		$pageCount_1 = $pdf->setSourceFile($filename);
