<?php
	...
	if(@$_POST["do"]=="crop") {
		if(!empty($_POST["img"])) {
			...
			$output_dir = $_SERVER["DOCUMENT_ROOT"].PROJECTPATH."/assets/uploads/tmp/";
			$filename = create_file_name($output_dir, $_POST["img"]);
...
<div style="text-align: center;">
	<?php if(!empty($_COOKIE["IMG"])) {
		...
		<form id="frmCrop" name="frmCrop" action="" method="post" enctype="multipart/form-data">
			<img src="<?php echo base_url().PROJECTPATH; ?>/assets/uploads/tmp/<?php echo $_COOKIE["IMG"]; ?>" id="cropbox" width="<?php echo $_COOKIE["IMG_W"]; ?>" height="<?php echo $_COOKIE["IMG_H"]; ?>" />
	...
	<form id="frm-upload" name="frm-upload" action="" method="post" enctype="multipart/form-data">
		<div class="form-group">
			<label></label>
			<input type="file" id="request_file" name="request_file" class="form-control" style="height: auto;" />
		</div>
