...
<h1 class="title_top">ยกเลิกรายการชำระหนี้คงค้าง</h1>
        ...
        <div class="panel panel-body">
            <div class="row"  style="padding-bottom:15px !important;">
             ...
             <table class="table table-bordered table-striped table-center">
                ...
                ?>
					<tr> 
                        <td><?php echo $row['non_pay_year'].'/'.$month_arr[$row['non_pay_month']]; ?></td>
                        <td><?php echo $row['member_id']; ?></td> 
                        <td class="text-left"><?php echo $row['prename_full'].$row['firstname_th'].'  '.$row['lastname_th'];?></td>											
                        <td><?php echo $this->center_function->ConvertToThaiDate($row['updatetimestamp'],1,0);?></td>
...
<form action="<?php echo base_url(PROJECTPATH.'/debt/coop_non_pay_receipt_cancel'); ?>" id="cancel_form" method="GET">
...
<script>
	function cancel(receipt_id){
        ...
        function(isConfirm) {
            if (isConfirm) {
				//document.location.href = base_url+'/debt/coop_non_pay_receipt_cancel?receipt_id='+receipt_id;
    ...
    function check_search() {
		if ($('#search_list').val() == '') {
            ...
            document.location.href = base_url +'debt/coop_non_pay_receipt_cancel?search_text='+$('#search_text').val()+"&search_list="+$('#search_list').val();
		}
	}
