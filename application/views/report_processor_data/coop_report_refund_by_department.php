...
?>
		<div style="min-width: 1500px; display: table;"  class="page-break">
			<div class="panel panel-body" style="padding-top:10px !important;min-height: 950px;">
				...
				?>	
					<tr>
						...
						<img src="<?php echo base_url(PROJECTPATH.'/assets/images/coop_profile/'.$_SESSION['COOP_IMG']); ?>" alt="Logo" style="height: 80px;" />
							...
							<h3 class="title_view">สรุปรายการคืนเงินแยกตามหน่วยงาน</h3>
				...
				<table class="table table-view table-center" style="max-width: 100%;">
					<thead>
						<tr>
                            <th rowspan="3" style="width:40px; vertical-align: middle;">ลำดับ</th>
                            <th rowspan="3" style="width:400px;vertical-align: middle;">หน่วยงาน</th>
							<?php
					...
					<?php
						...
						foreach($header_type as $loan_type_code => $heads) {
							foreach($heads as $loan_name_id => $head) {
								$interest = "";
								$principal = "";
								if (!empty($body_data[$row['id']][$loan_type_code][$head['id']]['interest'])){
									$interest = $body_data[$row['id']][$loan_type_code][$head['id']]['interest'];
									$row_total += $interest;
									$col_total[$loan_type_code][$head['id']]['interest'] += $interest;
								}
								if (!empty($body_data[$row['id']][$loan_type_code][$head['id']]['principal'])) {
									$principal = $body_data[$row['id']][$loan_type_code][$head['id']]['principal'];
									$row_total += $principal;
									$col_total[$loan_type_code][$head['id']]['principal'] += $principal;
								}
								
					?>
							<td style="text-align: right;"><?php echo !empty($principal) ? number_format($principal,2) : "0.00"; ?></td>
							<td style="text-align: right;"><?php echo !empty($interest) ? number_format($interest,2) : "0.00"; ?></td>
					<?php
									}
								}
								$all_total += $row_total;
