	...
	for ($pageNo = 1; $pageNo <= $pageCount_1; $pageNo++) {	
        $pdf->AddPage();
            ...
            if($pageNo == '1'){
                ...
                $pdf->MultiCell(58, 5, U2T($data['coop_profile']['coop_name_th']), $border, "C"); //เขียนที่
                $y_point = 53.5;
                ...
                $pdf->MultiCell(70, 5, U2T(''), $border, "C"); //เขต, อำเภอ
                ...
                $pdf->MultiCell(45, 5, U2T($data['mem_group_name_level']), $border, "C"); //สังกัด

                $y_point = 130;
                $pdf->SetXY( 57, $y_point );
                $pdf->MultiCell(37, 5, U2T($data['c_address_no']), $border, "C"); //บ้านเลขที่
                $pdf->SetXY( 105, $y_point );
                $pdf->MultiCell(39, 5, U2T(($data['c_address_road']=='')?'-':$data['c_address_road']), $border, "C"); //ถนน..
                $pdf->SetXY( 165, $y_point );
                $pdf->MultiCell(32, 5, U2T(($data['c_district_name']=='')?'-':$data['c_district_name']), $border, "C"); //แขวง, ตำบล..
                $y_point = 137.8;
                $pdf->SetXY( 41, $y_point );
                $pdf->MultiCell(52, 5, U2T($data['c_amphur_name']), $border, "C"); //เขต, อำเภอ
                $pdf->SetXY( 107, $y_point );
                $pdf->MultiCell(38, 5, U2T($data['c_province_name']), $border, "C"); //จังหวัด
                ...
                $pdf->MultiCell(35, 5, U2T($full_date), $border, "L"); //วันที่
                $y_point = 50.6;
                $pdf->SetXY( 160, $y_point );
                $pdf->MultiCell(32, 5, U2T($data['member_id']), $border, "C"); //ตำแหน่ง
                $y_point = 58.2;
                ...
                $pdf->MultiCell(43, 5, U2T($data['member_id']), $border, "C"); //สังกัด
                $pdf->SetXY( 172.5, $y_point );
                $pdf->MultiCell(24, 5, U2T($data['c_address_no']), $border, "C"); //บ้านเลขที่
                $y_point = 73.5;
                $pdf->SetXY( 30.5, $y_point );
                $pdf->MultiCell(47, 5, U2T(($data['c_address_road']=='')?'-':$data['address_road']), $border, "C"); //ถนน
                $pdf->SetXY( 97, $y_point );
                $pdf->MultiCell(40, 5, U2T($data['c_district_name']), $border, "C"); //ตำบล
                $pdf->SetXY( 155, $y_point );
                $pdf->MultiCell(38, 5, U2T($data['c_amphur_name']), $border, "C"); //อำเภอ
                $y_point = 81;
                $pdf->SetXY( 35, $y_point );
                $pdf->MultiCell(40, 5, U2T($data['c_province_name']), $border, "C"); //จังหวัด
                $pdf->SetXY( 97, $y_point );
                ...
                $pdf->MultiCell(27, 5, U2T($month_arr[$month_start]), $border, "C"); //ตั้งแต่เดือน
            }else if($pageNo == '4'){
                $y_point = 27;
                $pdf->SetXY( 120, $y_point );
                $pdf->MultiCell(12, 5, U2T(''), $border, "C"); //สหกรณ์เลขที่
                $pdf->SetXY( 145, $y_point );
                $pdf->MultiCell(14, 5, U2T(''), $border, "C"); //จำนวน
                $pdf->SetXY( 175, $y_point );
                $pdf->MultiCell(14, 5, U2T(''), $border, "C"); //เป็นจำนวนเงิน
                ...
                $pdf->MultiCell(34, 5, U2T($data['loan_amount']), $border, "C"); //จำนวนเงิน
                $pdf->SetXY( 119, $y_point );
                $pdf->MultiCell(65, 5, U2T($money_loan_amount_2text), $border, "C"); //จำนวนเงินตัวเขียน
                // $data['approve_date']substr($data['date_start_period'], 8, 2)
                $y_point = 98;
                $pdf->SetXY( 91, $y_point );
                $pdf->MultiCell(25, 5, U2T(substr($data['approve_date'], 0, 10)), $border, "C"); //
                $y_point = 113;
                $pdf->SetXY( 40, $y_point );
                $pdf->MultiCell(30, 5, U2T($data['loan_amount']), $border, "C"); //จำนวนเงิน
                $pdf->SetXY( 85, $y_point );
                ...
                $pdf->MultiCell(14, 5, U2T($date_to_year), $border, "C"); //ปี
                $y_point = 55.8;
                ...
                $pdf->MultiCell(15, 5, U2T(($data['c_address_moo']=='')?'-':$data['c_address_moo']), $border, "C"); //
                $pdf->SetXY( 106.5, $y_point );
                $pdf->MultiCell(42, 5, U2T(($data['c_address_soi']=='')?'-':$data['c_address_soi']), $border, "C"); //
                $pdf->SetXY( 157.5, $y_point );
                $pdf->MultiCell(38, 5, U2T(($data['c_address_road']=='')?'-':$data['c_address_road']), $border, "C"); //
                $y_point = 71;
                $pdf->SetXY( 40.5, $y_point );
                $pdf->MultiCell(48, 5, U2T($data['c_district_name']), $border, "C"); //
                $pdf->SetXY( 106, $y_point );
                $pdf->MultiCell(42, 5, U2T($data['c_amphur_name']), $border, "C"); //
                $pdf->SetXY( 161, $y_point );
                $pdf->MultiCell(35, 5, U2T($data['c_province_name']), $border, "C"); //
                $y_point = 78.5;
                $pdf->SetXY( 49, $y_point );
                ...
                $y_point = 93.8;
                $pdf->SetXY( 157.5, $y_point );
                $pdf->MultiCell(40, 5, U2T($data['member_id']), $border, "C"); //
            }else if($pageNo == '8'){
                ...
                $pdf->MultiCell(65, 5, U2T($data['position']), $border, "C"); //ชื่อ...
                $y_point = 95.8;
                $pdf->SetXY( 41, $y_point );
                $pdf->MultiCell(62, 5, U2T($data['mem_group_name_faction']), $border, "C"); //ชื่อ...
                $pdf->SetXY(115, $y_point );
                $pdf->MultiCell(50, 5, U2T($data['mem_group_name_level']), $border, "C"); //ชื่อ...
                $y_point = 111;
                $pdf->SetXY( 35.5, $y_point );
                $pdf->MultiCell(23, 5, U2T($data['salary']), $border, "C"); //
                $pdf->SetXY( 70, $y_point );
                $pdf->MultiCell(64, 5, U2T($this->center_function->convert($data['salary'])), $border, "C"); //
                $pdf->SetXY( 159, $y_point );
                $pdf->MultiCell(38, 5, U2T($month_arr[$data['month_start']]), $border, "C"); //date_to_month
                $y_point = 118.7;
                $pdf->SetXY( 51, $y_point );
                $pdf->MultiCell(40, 5, U2T($data['salary']-$data['money_per_period']), $border, "C"); //
